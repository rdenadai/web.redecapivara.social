  ...
  <div class="min-h-screen bg-capivara-off-white">
    ...
    <main class="max-w-7xl mx-auto md:px-4 md:py-4">
      <div class="card mb-8 bg-gradient-to-r text-white">
        <div>
          <h1 class="text-2xl font-bold mb-2 text-black/80 px-4 py-4">Seguindo ({{ statsData.following }})</h1>
          <div class="mt-6 space-y-4">
            ...
            <div v-if="noMore && following.length > 0" class="text-center py-4 text-capivara-stone/60">
              Fim da lista
            </div>
          </div>
        </div>
...
async function fetchInitialFollowing() {
  ...
  try {
    ...
    try {
      ...
      console.warn('Não foi possível carregar seguidores para mutual badge', err)
    }
  } catch (err) {
...
const loadMoreFollowing = async () => {
  ...
  loadingMore.value = true
  try {
    const more = await getFollows(authStore.server, authStore.accessToken, authStore.did, 100, cursor.value)
