      ...
      <div class="card mb-8 bg-gradient-to-r text-white">
        <div>
          ...
          <div class="mt-2 space-y-4">
              ...
              <div class="flex items-start space-x-4">
                ...
                <div>
                  <h2 class="text-lg font-semibold text-black/80">
                    <router-link :to="`/${person?.handle}/profile`" class="hover:underline">{{
                  ...
                  <p>
                    <span class="text-xs text-capivara-stone/60 font-mono">DID: {{ person.did }}</span>
                  </p>
                  <p
            ...
            <div v-if="noMore && followers.length > 0" class="text-center py-4 text-capivara-stone/60">
              Fim da lista
            </div>
          </div>
        </div>
...
async function fetchInitialFollowers() {
  ...
  try {
    const response = await getFollowers(authStore.server, authStore.accessToken, did.value || authStore.did, 100)
    ...
    try {
      const resp2 = await getFollows(authStore.server, authStore.accessToken, did.value || authStore.did, 100)
      ...
      console.warn('Não foi possível carregar lista de seguidos para detectar seguimentos mútuos', err)
    }
  } catch (err) {
...
const loadMoreFollowers = async () => {
  ...
  loadingMore.value = true
  try {
    const moreFollowers = await getFollowers(authStore.server, authStore.accessToken, authStore.did, 100, cursor.value)
