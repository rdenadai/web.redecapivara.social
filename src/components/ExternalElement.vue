<template>
  <div class="w-full max-w-xl gap-2">
    <!-- image preview -->
    <div v-if="external && external.uri && /\.(gif|jpe?g|png)/i.test(external.uri)" class="mt-2">
      <img
        :src="external.uri"
    ...
    <div v-else-if="external" class="mt-2">
      <div class="bg-white border rounded shadow p-4 max-w-full overflow-hidden">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            <a
              ...
              class="block text-lg font-semibold text-capivara-brown hover:underline break-all"
            >
              {{ external.title || external.name || external.uri }}
            </a>

            <div v-else class="block text-lg font-semibold text-gray-800 break-words">
              {{ external.title || external.name || 'External content' }}
            </div>

            <p v-if="external?.description" class="mt-1 text-sm text-gray-600 line-clamp-3 break-words">
        ...
        <div v-if="external?.uri" class="mt-4 w-full max-w-full overflow-hidden">
          <div class="relative w-full max-w-full" style="padding-bottom: 48%; max-height: 12rem">
            <iframe
              :src="external.uri"
