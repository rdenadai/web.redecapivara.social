        ...
        <ul v-if="!loading && !error && notifications.length > 0">
          <li v-for="notification in notifications" :key="notification.id">
            <div class="p-4 hover:bg-gray-50 cursor-pointer">
              <router-link
                :to="parseNotificationPostUrl(notification)"
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    >
                      <div class="flex">
                        ...
                        <div
                          v-for="item in notification.grouped.slice(0, 5)"
                          :key="item.id"
                        >
                          <div
                            class="flex items-center space-x-3 text-capivara-brown"
                          >
                      ...
                      <div class="mt-2 pl-13">
                        <strong class="truncate">
                          {{
                            notification.grouped[0].author?.displayName ||
                            notification.grouped[0].author?.handle
                          }}
                        </strong>
                        e
                        <span class="items-center font-semibold">{{
                          notification.grouped.length - 1
                        }}</span>
                        ...
                        <span class="text-gray-500 text-xs">
                          •
                          {{
                            numberOfHoursOrMinutesAgo(
                              notification.grouped[0].record.createdAt
                            ) + " atrás"
                          }}
                        </span>
                      </div>
                    ...
                    <div
                      v-else
                      class="flex flex-col items-start space-x-3 text-capivara-brown"
                    >
                      <div class="flex items-center text-capivara-brown">
                        <NotificationIcons :reason="notification.reason" />
                        <div
                          v-if="notification.grouped[0].author?.avatar"
                          class="pl-1"
                        >
                          <img
...
const allNotifications = computed(() =>
  notifications.value.reduce((acc, notif) => acc + notif.grouped.length, 0)
);
    ...
    if (response && Array.isArray(response.notifications)) {
      ...
      response.notifications.forEach((notif) => {
        ...
        if (!groupedMap.has(key)) {
          ...
          if (
            new Date(notif.createdAt) > new Date(groupedMap.get(key).createdAt)
          ) {
  ...
  if (notifications.value.length > 0) {
    for (const notif of notifications.value) {
      ...
      if (item.isRead === false) {
        ...
        console.log("Updating seenAt to:", new Date(Date.now()).toISOString());
        if (latestCreatedAt) {
